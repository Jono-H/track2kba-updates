...
#'
tripSplit_maptools_removed_rbind <- function(
  ...
  for(nid in seq_len(length(unique(dataGroup$ID)))) {
    ...
    if (nid == 1) {Trips <- TrackOut} else {
      ...
      ## 17 March 2023 - 11:55
      ## Removing maptools. Originally - the code provided was this version with maptools called directly on the spRbind function
      #Trips <- maptools::spRbind(Trips, TrackOut)
      
      ## 17 March 2023 - 12:14
      ## Keeping the function in but removing the direct call to maptools
      #Trips <- spRbind(Trips, TrackOut)
      
      ## 17 March 2023 - 12:48
      ## removing the function and just changing to rbind
